    ...
    for b in range(nB):
        for t in range(50):
            ...
            for n in range(nA):
                ...
                iou  = bbox_iou_v2(anchor_box, gt_box, x1y1x2y2=False)
                if anchor_step == 4:
            ...
            # gt_box = [b.cpu() for b in gt_box]

            iou = bbox_iou_v2(gt_box, pred_box.float(), x1y1x2y2=False) # best_iou
...
class RegionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        nGT, nCorrect, coord_mask, conf_mask, cls_mask, tx, ty, tw, th, tconf,tcls = build_targets(pred_boxes, target, self.anchors, nA, nC, \
        ...
        return loss, torch.stack((loss_x, loss_y, loss_w, loss_h, loss_conf, loss_cls, loss)).detach()
